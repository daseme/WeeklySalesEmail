...
def get_token_and_team_member_id():
    ...
    team_member_id = os.environ.get("DROPBOX_TEAM_MEMBER_ID", "dbmid:AACRH2RkF_4U4f9kEEGcmzLfwC6a9Z1JLIw")  # Default to Kurt's ID
...
def create_folder(token, team_member_id, path):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Error creating folder {path}: {response.status_code} - {response.text}")
            return False
...
def upload_reports(token, team_member_id, config):
    ...
    dropbox_reports_path = config.get("dropbox_reports_folder", "/Financial/Sales/WeeklyReports/reports")
...
def upload_logs(token, team_member_id, config):
    ...
    dropbox_logs_path = config.get("dropbox_logs_folder", "/Financial/Sales/WeeklyReports/reports/logs")
...
def main():
    ...
    try:
        ...
        config["dropbox_templates_path"] = "/Financial/Sales/WeeklySalesEmail/email_templates"
