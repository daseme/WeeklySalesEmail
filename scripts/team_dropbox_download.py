...
def list_folder(token, team_member_id, path):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Error listing {path}: {response.status_code} - {response.text}")
            return []
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as temp:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error downloading {dropbox_path}: {response.status_code} - {response.text}")
...
def download_vba_file(token, team_member_id, config):
    ...
    vba_path = config.get("dropbox_vba_path", "/Financial/Sales/WeeklyReports/vbaProject.bin")
